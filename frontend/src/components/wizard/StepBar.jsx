    ...
    <div className="min-w-0 w-full flex-shrink-0 overflow-x-auto overscroll-x-contain border-b border-white/50 bg-white/40 [scrollbar-width:thin]">
      <div className="flex h-[50px] w-max min-w-full items-center justify-between gap-0 px-4 sm:px-5">
          ...
          {steps.map((s, i) => {
            ...
            return (
              <div key={num} className="flex items-center">
                {i > 0 && <span className="px-0.5 text-sm text-slate-300 select-none">›</span>}
                <button
        ...
        <button
          ...
          className="flex-shrink-0 rounded-xl border border-slate-200/80 bg-white/70 px-4 py-[7px] text-xs font-semibold text-slate-500 hover:bg-white/90 disabled:opacity-50"
        >
          Salvesta mustand
        </button>
      </div>
